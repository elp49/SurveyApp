...
public class SerializationHelper {
     ...
     */
    public static <T> T deserialize(Class<T> type, String path) throws IOException, ClassNotFoundException {
        FileInputStream fis;
        ObjectInputStream ois;
        T deserializedObject;

        // Test the path exists and is a file.
        File tst = new File(path);
        if (!tst.exists() || !tst.isFile())
            throw new IllegalArgumentException(path + " is invalid");

        // Create streams to read it in.
